...
def predict_proba(model, X, y, num_classes=10, class_one_hot_coded=True):
    result = np.zeros((X.shape[0], num_classes))
    shadow_preds = X#[:, : num_classes]
    classes = y#X[:, num_classes :]

    data_indices = np.arange(shadow_preds.shape[0])
    for i in range(num_classes):
        # model = self._get_model(i)
        if class_one_hot_coded:
...
def prepare_attack_data(model, data_in, data_out):
    ...
    y_hat_in = predict_proba(model, X_in, y_in)
    y_hat_out = predict_proba(model, X_out, y_out)

    # np.savetxt("in.csv", y_hat_in, delimiter=";")
    # np.savetxt("out.csv", y_hat_out, delimiter=";")

    # import matplotlib.pyplot as plt
    # plt.plot(y_hat_in.mean(axis=0))
    # plt.plot(y_hat_out.mean(axis=0))
    # plt.show()
    # exit()

    data = np.vstack([y_hat_in, y_hat_out])
    labels = np.hstack([np.ones(y_hat_in.shape[0]), np.zeros(y_hat_out.shape[0])])
    # data = np.hstack([np.vstack([y_hat_in, y_in]), np.vstack([y_hat_out, y_out])])
    return data, labels
...
class AttackModelBundle(sklearn.base.BaseEstimator):
    ...
    def __init__(
        self, model_fn, num_classes, serializer=None, class_one_hot_coded=False
    ):
        self.model_fn = model_fn
    ...
    def predict(self, X, y):
        probs = self.predict_proba(X, y)[:, 1]
        return probs > 0.5
